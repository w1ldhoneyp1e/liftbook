...
`push` ограничен 50 изменениями за запрос. Пустой batch и некорректный ISO `updatedAt` должны получать `400`.
...
Если изменений больше, чем сервер возвращает за один ответ, API ставит `hasMore: true`. В этом случае клиент должен сразу выполнить следующий `pull` с новым `nextCursor`, пока `hasMore` не станет `false`.
