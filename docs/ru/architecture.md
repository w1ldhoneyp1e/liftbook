...
Первый sync UI находится в Settings, а клиент также пытается синхронизироваться автоматически, когда устройство онлайн и guest session уже существует. Если есть pending local records, он сначала отправляет их в API. Даже если pending local records нет, клиент все равно может делать `pull`, чтобы подтянуть изменения с других устройств, и сохраняет cursor в локальной account session.

Простые конфликты сейчас разрешаются системой автоматически по `updatedAt`. Пользовательский UI для ручного разбора конфликтов не входит в основной MVP-сценарий.
